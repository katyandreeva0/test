...
// Layout 2: BENTO GRID - Mixed sized cards in a Pinterest-style masonry layout
export default function BenefitsLayout2() {
  const content = landingPageContent;
  const colors = ['blue', 'green', 'purple', 'orange', 'teal'];
  const icons = [BookOpen, Brain, MessageCircle, BarChart3, GraduationCap];

  return (
    <section id="features" className="px-4 sm:px-6 lg:px-8 py-16 lg:py-24 bg-gradient-to-b from-white to-gray-50">
      <div className="mx-auto max-w-7xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-16">
          {content.benefits.usps.map((usp, index) => {
            ...
            const colorClasses = [
              { bg: 'bg-blue-600', badge: 'bg-blue-50 text-blue-700 border-blue-200' },
              { bg: 'bg-green-600', badge: 'bg-green-50 text-green-700 border-green-200' },
              { bg: 'bg-purple-600', badge: 'bg-purple-50 text-purple-700 border-purple-200' },
              { bg: 'bg-orange-600', badge: 'bg-orange-50 text-orange-700 border-orange-200' },
              { bg: 'bg-teal-600', badge: 'bg-teal-50 text-teal-700 border-teal-200' }
            ];
            ...
            return (
              <Card key={index} className={`${gridClasses[index]} p-6 bg-white border border-gray-200 hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1`}>
                <div className="flex items-start gap-4 mb-4">
                  <div className={`${colorClass.bg} rounded-xl p-3 shadow-lg`}>
                  ...
                  <div className="flex-1">
                    ...
                    {/* Show different amounts of features based on card size */}
                    <div className="space-y-2">
                      {usp.features.slice(0, index === 4 ? 3 : 2).map((feature, featureIndex) => (
        ...
        <div className="relative">
          ...
          <div className="relative bg-white/95 backdrop-blur-sm rounded-3xl p-8 m-1 text-center border">
            ...
            <p className="text-sm text-gray-600 mt-4">Join 10,000+ early learners • No commitment</p>
          </div>
        </div>
      </div>
    </section>
